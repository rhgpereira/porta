...
When "I fill in all required signup fields as {string}" do |buyer_name|
...
When "I fill in the invitation signup with email {string}" do | email |
  fill_in('Email', with: email)
  step %(I fill in the invitation signup as "#{email}")
end

When "I fill in the invitation signup as {string}" do |username|
...
When "new provider {string} signs up and activates" do |name|
...
When "I select the {string} application plan" do |plan|
  pending
end
...
#FIXME
Then "{string} should receive an email to activate the account" do |email_address|
  # pending
